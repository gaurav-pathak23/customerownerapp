...
import { View, Text, Image, StyleSheet, Dimensions, ScrollView, TouchableOpacity ,Modal,FlatList} from 'react-native';
        ...
        renderItem={({ item }) => (
          ...
          >
            <Text
              style={[
                styles.text,
                value?.toString() === item?.toString() && styles.selectedText,
              ]}
            >
              {item}
            </Text>
...
<View style={{flexDirection:"row",justifyContent:"space-between",
    paddingRight:mobileWidth*8/100,paddingLeft:mobileWidth*8/100}}>
      ...
      <View style={{flexDirection:"row",width:mobileWidth*60/100,}}>
        <View style={{padding:mobileWidth*1/100}}>
           <Text style={{fontSize:mobileWidth*4/100,fontWeight:"700"}}>{item.platerupees}</Text>
          <Text  style={{color:Colors.layercolor,fontSize:mobileWidth*3.5/100,marginTop:mobileWidth*2/100}}>{item.name}</Text>
          </View>

<Text style={{marginTop:mobileWidth*4/100,fontWeight:"700",
    left:mobileWidth*4/100,fontSize:mobileWidth*4.5/100,color:Colors.blacktxt}}>  {item.rupees}{'\n'}
      ...
      {Array.from({ length: 5 }).map((_, i) => (
        <Image
          ...
          source={
            i < item.rating
              ? require('../src/Icons/satrfull.png')    // Red or colored star
              : require('../src/Icons/starrrr.png')  // Gray or empty star
          }
          style={styles.starIcon}
        />
      ))}
      <TouchableOpacity style={{width:mobileWidth*17/100,height:mobileWidth*7/100,left:mobileWidth*32/100,
        borderRadius:mobileWidth*5/100,backgroundColor:Colors.tabcolor,justifyContent:"center"}}>
      <Text style={{color:Colors.whitetxt,textAlign:"center",fontSize:mobileWidth*4/100}}>View</Text>
      </TouchableOpacity>
